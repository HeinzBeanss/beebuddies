                    ...
                    friends.map((user, index) => {
                        return (
                            <div className='friendlist-card' key={index}>
                                <Link className="friend-pfp" to={`/user/${user._id}`}><img className="friend-pfp" src={`data:${user.profile_picture.contentType};base64,${user.profile_picture.data}`} alt="Image" /></Link>
